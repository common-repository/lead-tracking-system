...
class ExportBase {
    ...
    protected function &getColumnsToDisplay($dataColumns) {
            ...
            //    $dataColumns[11]="hello";
 
         //  print_r($dataColumns);
            $dataColumns[0]="Sr.no" ;
			
			$dataColumns[1]="Submitted" ;
    ...
    public function &getPivotQuery($formName, $count = false) {
        ...
        foreach ($rows as $aRow) {
            if (!in_array($aRow->field_name, $fields)) {
               if($aRow->field_name!="_wpcf7" && $aRow->field_name!="_wpcf7_version" && $aRow->field_name!="_wpcf7_unit_tag" && $aRow->field_name!="_wpcf7_is_ajax_call")
